...
<AccordionGroup>
  ...
  <Accordion title="Linux: systemd service fails to start">
    ...
    Common causes:
    - **Missing config file**: Ensure `~/.bolly/data/config.toml` exists
    - **Invalid API key**: Check that your Anthropic key is correctly formatted
    - **Port in use**: Another service is using port 8080 — change the port or stop the conflicting service
...
<AccordionGroup>
  <Accordion title="Container exits immediately">
    ...
    Usually caused by:
    - Missing or invalid `config.toml` in the volume
    - Missing Anthropic API key
    - Port conflict (another container using 8080)
  </Accordion>

  <Accordion title="Can't access Bolly at localhost:8080">
...
<AccordionGroup>
  <Accordion title="'Invalid API key' error">
    Double-check your key in `config.toml`:
